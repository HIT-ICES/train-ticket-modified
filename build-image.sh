...
echo 'tag the image'
docker images | grep framework | awk '{print "docker tag"" "$1":"$2 " ""172.31.15.247:5000/"$1":"$2}'| sh

echo 'docker push to the registry'
docker images | grep 172.31.15.247:5000/framework | awk '{print "docker push"" "$1":"$2}' | sh
